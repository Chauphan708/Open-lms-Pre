        ...
        return (
            <div className="max-w-xl mx-auto text-center py-12 animate-fade-in">
                <div className="flex items-center justify-center gap-6 mb-8">
                    <div className="w-24 h-24 rounded-2xl flex items-center justify-center text-5xl shadow-lg bg-indigo-900 border-2 border-indigo-500 dark:border-slate-800">
                        {AVATAR_EMOJIS[arenaProfile?.avatar_class || 'scholar']}
                    </div>
                    <div className="animate-pulse">
                        <Brain className="h-12 w-12 text-indigo-500" />
                    </div>
                    <div className="w-24 h-24 rounded-2xl flex items-center justify-center text-5xl shadow-lg bg-gray-800 border-2 border-purple-500 dark:border-slate-800">
                        🧑‍🎓
                    </div>
                </div>
                <h2 className="text-3xl font-black text-gray-900 mb-4 animate-bounce dark:text-slate-100">VÀO TRẬN!</h2>
    ...
    if (isHosting) {
        return (
            <div className="max-w-md mx-auto text-center py-8 animate-fade-in">
                ...
                <div className="bg-white rounded-2xl border-2 border-indigo-100 p-8 shadow-xl shadow-indigo-100/50 relative overflow-hidden dark:bg-slate-900 dark:border-indigo-900/30">
                    <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-purple-500 to-indigo-500"></div>

                    {!challengerId ? (
                        <div className="space-y-6">
                            <div className="w-24 h-24 mx-auto rounded-full bg-indigo-50 flex items-center justify-center border-4 border-indigo-100 border-t-indigo-500 animate-spin dark:border-indigo-900/30">
                                <span className="text-4xl animate-none">{AVATAR_EMOJIS[arenaProfile?.avatar_class || 'scholar']}</span>
                            </div>
                            <div>
                                <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100">Đang chờ đối thủ thách đấu...</h3>
                                <p className="text-gray-500 text-sm mt-2 dark:text-slate-500">Người chơi khác sẽ thấy phòng của bạn ở sảnh chính.</p>
                            </div>
                            <button onClick={handleCancel} className="mt-4 px-6 py-2 bg-gray-100 text-gray-700 font-bold rounded-lg hover:bg-gray-200 dark:bg-slate-850 dark:text-slate-300">
                                Hủy phòng
                            </button>
                        </div>
                    ) : (
                        <div className="space-y-6 animate-in zoom-in-95">
                            <div className="bg-indigo-50 text-indigo-700 font-bold py-2 rounded-lg text-sm mb-4 border border-indigo-200 dark:border-indigo-900/30">
                                🧠 Có Lời Thách Đấu Mới!
                            </div>
                            <div className="flex items-center justify-center gap-4">
                                <div className="text-center">
                                    <h4 className="font-black text-xl text-gray-900 dark:text-slate-100">{getPlayerName(challengerId)}</h4>
                                    <p className="text-gray-500 text-sm dark:text-slate-500">Muốn thách đấu kiến thức với bạn</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-3 pt-4 border-t dark:border-slate-800">
                                <button onClick={handleReject} className="py-3 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 dark:bg-slate-850 dark:text-slate-400">
                                    Từ chối
                                </button>
    ...
    if (isChallenging) {
        return (
            <div className="max-w-md mx-auto text-center py-12 animate-fade-in">
                <div className="w-24 h-24 mx-auto rounded-full bg-indigo-50 flex items-center justify-center border-4 border-indigo-100 border-t-indigo-500 animate-spin mb-6 dark:border-indigo-900/30">
                    <Brain className="h-10 w-10 text-indigo-500 animate-none" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-2 dark:text-slate-100">Đã gửi lời thách đấu!</h3>
                <p className="text-gray-500 mb-8 dark:text-slate-500">Vui lòng chờ chủ phòng chấp nhận...</p>
                <button onClick={handleCancel} className="px-6 py-2 bg-gray-100 text-gray-700 font-bold rounded-lg hover:bg-gray-200 dark:bg-slate-850 dark:text-slate-300">
    ...
    return (
        <div className="max-w-3xl mx-auto animate-fade-in">
            <div className="flex items-center justify-between mb-8">
                <button onClick={() => navigate('/arena')} className="flex items-center gap-2 text-gray-500 hover:text-gray-700 font-medium dark:text-slate-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-4">
                    ...
                    {matches.filter(m => m.player1_id === user?.id).length > 0 && (
                        <div>
                            <h3 className="font-bold text-gray-700 mb-2 dark:text-slate-300">📌 Phòng của bạn</h3>
                            {matches.filter(m => m.player1_id === user?.id).map(match => (
                                <div key={match.id} className="bg-indigo-50 border-2 border-indigo-200 rounded-xl p-4 flex items-center justify-between dark:border-indigo-900/30">
                                    <div>
                    ...
                    <h3 className="font-bold text-gray-700 flex items-center justify-between dark:text-slate-300">
                        Phòng của người khác ({matches.filter(m => m.player1_id !== user?.id).length})
                    </h3>

                    {matches.filter(m => m.player1_id !== user?.id).length === 0 ? (
                        <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center dark:bg-slate-900 dark:border-slate-800">
                            <div className="text-4xl mb-4">📚</div>
                            <p className="text-gray-500 font-medium dark:text-slate-500">Sảnh đang vắng... Hãy nhấp "Tạo Phòng" để mời người khác!</p>
                        </div>
                    ) : (
                        matches.filter(m => m.player1_id !== user?.id).map(match => (
                            <div key={match.id} className="bg-white border rounded-xl p-4 flex items-center justify-between hover:border-indigo-300 hover:shadow-md transition-all group dark:bg-slate-900 dark:border-slate-800">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center text-2xl group-hover:bg-indigo-50 transition-colors dark:bg-slate-850">
                                        🧑‍🎓
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-gray-900 group-hover:text-indigo-700 transition-colors dark:text-slate-100">
            ...
            {showCreateModal && (
                <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowCreateModal(false)}>
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md dark:bg-slate-900" onClick={e => e.stopPropagation()}>
                        <div className="p-6 border-b flex items-center justify-between dark:border-slate-800">
                            <h3 className="font-bold text-lg text-gray-900 dark:text-slate-100">🧠 Tạo Phòng Đấu Trí</h3>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <select value={filterSubject} onChange={e => setFilterSubject(e.target.value)}
                                    className="w-full border rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none dark:border-slate-800">
                            ...
                            {filterSource === 'exam' && availableGrades.length > 0 && (
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1 dark:text-slate-300">Khối lớp</label>
                            ...
                            {/* Info */}
                            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 text-xs text-indigo-700 dark:border-indigo-900/30">
                                {filterSource === 'exam'
