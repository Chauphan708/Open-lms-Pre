...
export type QuestionType = 'MCQ' | 'MATCHING' | 'ORDERING' | 'DRAG_DROP' | 'SHORT_ANSWER' | 'MCQ_MULTIPLE' | 'SENTENCE_SCRAMBLE' | 'WORD_CLASSIFY' | 'FILL_IN_PASSAGE' | 'INLINE_DROPDOWN';
...
export interface ArenaQuestion {
  id: string;
  content: string;
  answers: string[]; // 4 đáp án (rỗng đối với SHORT_ANSWER)
  correct_index?: number; // 0-3 (hoặc default, tùy chọn)
...
export interface AppState {
  ...
  fetchDiscussionMessages: (sessionId: string, limitCount?: number) => Promise<void>;
  ...
  fetchArenaQuestions: (filters?: { subject?: string; difficulty?: number; grade?: string; topic?: string; type?: string; search?: string }, includeStats?: boolean) => Promise<void>;
  loadMoreArenaQuestions: (filters?: { subject?: string; difficulty?: number; grade?: string; topic?: string; type?: string; search?: string }) => Promise<void>;
