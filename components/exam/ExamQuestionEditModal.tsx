...
export const ExamQuestionEditModal: React.FC<ExamQuestionEditModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border dark:border-slate-800">
        <div className="p-5 border-b dark:border-slate-800 flex justify-between items-center sticky top-0 bg-white dark:bg-slate-900 z-10">
          <h3 className="font-bold text-lg text-gray-800 dark:text-slate-200">Chỉnh sửa câu hỏi</h3>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-bold text-gray-700 dark:text-slate-355 mb-1">Nội dung câu hỏi</label>
            <textarea
              value={editingQuestion.content}
              onChange={e => setEditingQuestion({ ...editingQuestion, content: e.target.value })}
              className="w-full border border-gray-300 dark:border-slate-800 rounded-lg p-3 bg-white dark:bg-slate-950 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none h-24"
            />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-1">Loại câu hỏi</label>
              <select
                ...
                className="w-full border border-gray-300 dark:border-slate-800 rounded-lg p-3 bg-white dark:bg-slate-950 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none text-sm"
              >
            ...
            <div>
              ...
              />
              <p className="text-[10px] text-gray-400 dark:text-slate-505 mt-1 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-1">Mức độ (Tùy chọn)</label>
              <select
            ...
            <div className="flex items-center mt-6">
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-gray-700 dark:text-slate-300 group-hover:text-gray-900 dark:group-hover:text-slate-100">Không tính điểm phần này</span>
                  <span className="text-[10px] text-gray-500 dark:text-slate-400 italic block mt-0.5">Dành cho câu hỏi khảo sát, thu thập thông tin</span>
                </div>
              </label>
            </div>
          </div>
          ...
          {editingQuestion.imageUrl && (
            <div className="mt-2 p-2 border dark:border-slate-800 rounded-lg bg-gray-50 dark:bg-slate-950/40">
              <p className="text-xs text-gray-500 dark:text-slate-400 mb-1">Xem trước:</p>
              {imageError ? (
                <div className="flex items-center gap-2 p-3 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/40 rounded-lg">
                  ...
                  <div>
                    <p className="text-sm font-bold text-red-700 dark:text-red-400">Không tải được ảnh!</p>
                    <p className="text-xs text-red-600 dark:text-red-450 mt-0.5">
                ...
                <img 
                  ...
                  alt="Preview" 
                  className={`h-32 object-contain rounded border dark:border-slate-800 bg-white dark:bg-slate-950 transition-opacity ${imageLoaded ? 'opacity-100' : 'opacity-30'}`}
          ...
          {(editingQuestion.type === 'MCQ' || editingQuestion.type === 'MCQ_MULTIPLE') && (
            <div>
              <label className="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2 flex items-center justify-between">
                Các lựa chọn
                <button onClick={() => setEditingQuestion({ ...editingQuestion, options: [...editingQuestion.options, 'Lựa chọn mới'] })} className="text-xs text-indigo-600 dark:text-indigo-400 font-medium hover:underline">+ Thêm tùy chọn</button>
              </label>
              {editingQuestion.options.map((opt, i) => (
                <div key={i} className="flex items-center gap-2 mb-2 group">
                  ...
                  <span className="font-bold w-6 text-gray-500 dark:text-slate-400 text-center">{String.fromCharCode(65 + i)}</span>
                  <input
                    value={opt}
                    onChange={e => {
                    ...
                    className="flex-1 border border-gray-300 dark:border-slate-800 rounded-lg p-2 bg-white dark:bg-slate-950 text-gray-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                  <button onClick={() => {
                    const newOpts = [...editingQuestion.options];
                    newOpts.splice(i, 1);
                  ...
                  }} className="p-2 text-gray-400 dark:text-slate-500 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {['MATCHING', 'ORDERING', 'DRAG_DROP'].includes(editingQuestion.type) && (
            <div>
              ...
              {editingQuestion.type === 'DRAG_DROP' && (
                <p className="text-xs text-gray-500 dark:text-slate-400 mb-2 italic">Ghi chú: Trong phần "Nội dung câu hỏi" ở trên, dùng <code className="bg-gray-100 dark:bg-slate-800 dark:text-slate-300 px-1 rounded">[__]</code> để đánh dấu ô trống học sinh cần kéo thả/điền chữ vào.</p>
              )}
              {editingQuestion.options.map((opt, i) => (
                <div key={i} className="flex items-center gap-2 mb-2 group">
                  <span className="w-6 h-6 rounded bg-gray-100 dark:bg-slate-800 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-slate-400">{i + 1}</span>
                  <input
                    value={opt}
                    onChange={e => {
                      const newOpts = [...editingQuestion.options];
          ...
          {editingQuestion.type === 'SHORT_ANSWER' && (
            <div>
              <label className="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-1 flex items-center justify-between">
                ...
                <button onClick={() => setEditingQuestion({ ...editingQuestion, options: [...editingQuestion.options, 'Đáp án'] })} className="text-xs text-indigo-600 dark:text-indigo-400 font-medium hover:underline">+ Thêm đáp án</button>
              </label>
              <p className="text-xs text-gray-500 dark:text-slate-400 mb-2 italic">Hệ thống sẽ lấy danh sách này để so khớp tự động. Nếu trống, máy sẽ so khớp với nội dung Lời giải chi tiết.</p>
              {editingQuestion.options.map((opt, i) => (
                <div key={i} className="flex items-center gap-2 mb-2 group">
                  <span className="w-6 h-6 rounded bg-gray-100 dark:bg-slate-800 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-slate-400">{i + 1}</span>
                  <input
                    ...
                    className="flex-1 border border-indigo-300 dark:border-indigo-850 rounded-lg p-2 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-900 dark:text-indigo-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                  <button onClick={() => {
                    const newOpts = [...editingQuestion.options];
                    newOpts.splice(i, 1);
              ...
              {editingQuestion.options.length === 0 && (
                <div className="p-3 bg-yellow-50 dark:bg-yellow-950/20 text-yellow-800 dark:text-yellow-200 text-sm rounded-lg border border-yellow-200 dark:border-yellow-900/40">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-1">Gợi ý (Cách làm)</label>
              <textarea
                value={editingQuestion.hint || ''}
                onChange={e => setEditingQuestion({ ...editingQuestion, hint: e.target.value })}
                className="w-full border border-gray-300 dark:border-slate-800 rounded-lg p-3 bg-white dark:bg-slate-950 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none h-24 text-sm"
                placeholder="Hướng dẫn phương pháp giải..."
              />
            </div>
            <div>
              <label className="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-1">Lời giải chi tiết</label>
              <textarea
        ...
        <div className="p-5 border-t dark:border-slate-800 flex justify-end gap-2 bg-gray-50 dark:bg-slate-900/50 rounded-b-xl">
          <button onClick={() => setEditingQuestion(null)} className="px-4 py-2 text-gray-600 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-800 rounded-lg">Hủy</button>
